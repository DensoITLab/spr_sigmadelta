...
class Prophesee(NCaltech101):
    def __init__(self, root, object_classes, height, width, nr_events_window=-1, augmentation=False, mode='training',
        ...
        debug = False
        if debug:
            file_dir = os.path.join('detection_dataset_duration_60s_ratio_1.0', 'debug')
        else:
        ...
        if 1:
            ...
            path_sequence_start = './data/sequence_start_'+mode+'.bin'
            path_files = './data/files_'+mode+'.bin'

            if not os.path.exists(path_sequence_start):
                self.createAllBBoxDataset()
                with open(path_sequence_start, "wb") as fp:   #Pickling
                    pickle.dump(self.sequence_start, fp)
                with open(path_files, "wb") as fp:   #Pickling
                    pickle.dump(self.files, fp)
            else:
                with open(path_sequence_start, "rb") as fp:   #Pickling
                    self.sequence_start = pickle.load(fp)
    ...
    def createAllBBoxDataset(self):
        ...
        for i_file, file_name in enumerate(self.files):
            ...
            for unique_time in unique_ts:
                sequence_start0, sequence_start1 = self.searchEventSequence(event_file, unique_time, nr_window_events=250000, proc_rate=self.proc_rate)
                # sequence_start0, sequence_start1 = self.searchEventSequence(event_file, unique_time, nr_window_events=250000, dt=dt)
                # sequence_start1 = self.searchEventSequence(event_file, unique_time+dt, nr_window_events=250000)
                self.sequence_start.append((sequence_start0, sequence_start1))
    ...
    def __getitem__(self, idx):
        ...
        # Events
        events0 = self.readEventFile(event_file, self.sequence_start[idx][0],  nr_window_events=self.nr_events_window)
        events1 = self.readEventFile(event_file, self.sequence_start[idx][1],  nr_window_events=self.nr_events_window)
        histogram0 = self.generate_input_representation(events0, (self.height, self.width))
        histogram1 = self.generate_input_representation(events1, (self.height, self.width))

        return events0, events1, const_size_bbox.astype(np.int64), histogram0, histogram1

    def searchEventSequence(self, event_file, bbox_time, nr_window_events=250000, proc_rate=0):
        """
        Code adapted from:
        ...
        # cur_t = np.fromfile(file_handle, dtype=[('ts', 'u4'), ('_', 'i4')], count=1)["ts"][0]
        # next_t = cur_t.copy()
        # next_idx = 0


        if low + proc_rate >= nr_events:
            next_idx = nr_events
        else:
            next_idx = low + proc_rate

        # while next_t-cur_t< dt:
        #     if (low+next_idx)>=nr_events-1:
        #         break
        #     file_handle.seek(ev_start + (low+next_idx) * ev_size)
        #     next_t = np.fromfile(file_handle, dtype=[('ts', 'u4'), ('_', 'i4')], count=1)["ts"][0]
        #     next_idx = next_idx+1

        file_handle.close()
        # we now know that it is between low and high
        return ev_start + low * ev_size, ev_start + (next_idx) * ev_size
...
class NCars(NCaltech101):
    ...
    def __getitem__(self, idx):
        ...
        if 1:
            import matplotlib.pyplot as plt
            plt.imshow(histogram0[:,:,0])
            plt.savefig('histogram0_.png')
            plt.imshow(histogram1[:,:,0])
            plt.savefig('histogram1_.png')
